...
class Db:
    ...
    def get_session_factory(self):
        return sessionmaker(
            bind=self.engine, autoflush=True, expire_on_commit=True, future=True
        )
        ...
        try:
            ...
            if check_exist:
                ...
                existing_article = (
                    ...
                    .filter(Article.url == art.url or Article.id == art.id)
                    .first()
                )
    ...
    def get_articles(
        self, id: str = None, limit: int = 30, offset: int = 0
    ) -> List[Article]:
        try:
    ...
    def get_session(self):
        ...
        try:
            ...
            print_warning(
                f"[{self.tag}] Database connection lost: {e}. Reconnecting..."
            )
    ...
    def clean_expired_articles(self) -> int:
        """删除publish_at在当前时间点往前15天的文章, 返回删除数量"""
        session = self.get_session()
        try:
            threshold = datetime.now() - timedelta(days=15)
            deleted_count = (
                session.query(Article)
                .filter(Article.publish_at.isnot(None))
                .filter(Article.publish_at < threshold)
            ...
            session.rollback()
            print_error(f"DB.clean_expired_articles错误: {str(e)}")
            raise
